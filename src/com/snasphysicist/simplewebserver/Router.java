...
public class Router {
	...
	public void setErrorHandler( int statusCode, Function<Request,Response> handler ) {
		addRoute( String.format( "/%d.html", statusCode ), handler ) ;
	}
	
	private Response defaultResponse() {
		return new TextResponse( Protocol.HTTP10, 500, 
	...
	public Response route( Request request ) {
		...
		Function<Request,Response> handler = routes.get( request.getUri() ) ;
		if( handler == null ) {
			//Attempt to get response generator for 404 page 
			handler = routes.get( "/404.html" ) ;
			if( handler == null ) {
				//No route defined for uri or for 404 page, return default
				LOG.log( Level.WARNING, "No 404 route defined, returning default response" ) ;
				return defaultResponse() ;
			} else {
				//No route defined for uri, return 404
				LOG.log( Level.INFO, String.format( "No route found for %s, returning 404 response",
													request.getUri() ) ) ;
				return handler.apply( request ) ;
			}
		} else {
			//Route define for uri, generate & return appropriate response
			LOG.log( Level.INFO, String.format( "Route found for %s",
												request.getUri() ) ) ;
			return handler.apply( request ) ;
		}
	}
