...
{	
		...
		try {
			if(!handle()) {
				LOG.log(
					Level.WARNING,  
					String.format(
						"Failed to handle request from %s",
						getSourceIp()
					)
				);
			}
			...
			throws InterruptedException
	{
		...
		try {
			...
			LOG.log(
				Level.WARNING,
				"could not read incoming request, generic io error"
			); 
			return false;
		}
		...
		try {
			...
			LOG.log(
				Level.WARNING, 
				"Unsupported encoding in request"
			);
			return false;
		}
		...
		if(lines[0].split(" ").length != 3) {
			...
			LOG.log(
				Level.WARNING, 
				String.format(
					"Malformed first line of request %s",
					lines[0]
				)
			);
			return false;
