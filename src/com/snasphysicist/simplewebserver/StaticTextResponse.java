...
public class StaticTextResponse extends Response {

	private final static Logger LOG = Logger.getLogger( Logger.class.getName() ) ;
	
	URL filePath ;
	
	public StaticTextResponse( Protocol protocol , int status , String title , URL filePath ) {
		super( protocol , status , title ) ;
		this.filePath = filePath ;
	}

	public static ContentType guessContentType( URL filePath ) {
	...
	public static Header generateContentTypeHeader( ContentType contentType ) {
		...
		return new Header( "content-type" , contentTypeString ) ;
	...
	private void loadBody() {
		...
		try {
			...
			fileStream.close() ;
			body = new String( byteArrayListToArray( bytes ) , StandardCharsets.UTF_8 ) ;
			content += body ;
			LOG.log( Level.FINE , String.format( "Read body of length %d" , body.length() ) ) ;
		}
		catch( Exception e ) {
	...
	protected void generateContent() {
		addHeader( generateContentTypeHeader( guessContentType( filePath ) ) ) ;
		generateTopMatter() ;
		loadBody() ;
